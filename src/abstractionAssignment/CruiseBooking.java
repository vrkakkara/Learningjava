...
public class CruiseBooking extends MyBookings {
	...
	private double adultFoodPrice = 20.99;
	private double childFoodPrice = 4.99;
...
//Parameterized constructor which takes parameter values from corresponding child class
	...
	public CruiseBooking(boolean hasService,boolean hasFood, String customerName, int numberOfAdults, int numberOfBigKids) {
		super();
	...
	public double calculateCost() {
		...
		double serviceCost=((numberOfAdults+numberOfBigKids)*serviceRate);
		if (hasFood) {
		...
		double totalFare = adultFare + childFare + buffetAdults + buffetKids+serviceCost;
		...
		System.out.println("Buffet Special Price Kids above age 5 @" + numberOfBigKids + " : " + buffetKids);
		System.out.println("Your total service cost for"+(numberOfAdults+numberOfBigKids)+" people is "+serviceCost);
